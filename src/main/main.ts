...
ipcMain.handle('export-video', async (
  ...
  const encodeVideo = (videoBitrateKbps?: number) => {
    return new Promise<string | { status: 'canceled' }>((resolve, reject) => {
      ...
      if (type === 'mp3') {
        ...
        const videoOptions: string[] = [
          ...
          `-maxrate ${Math.max(200, Math.floor(videoBitrateKbps * 1.2))}k`,
          `-bufsize ${Math.max(400, Math.floor(videoBitrateKbps * 2.4))}k`,
        ...
        if (outputExt !== '.avi') {
          ...
          videoOptions.push('-x264-params "aq-mode=2:aq-strength=1.0:subme=6:trellis=1:ref=4:bframes=4:me=hex:merange=16"');
        }
  ...
  let videoBitrateKbps = Math.max(200, Math.floor(effectiveTargetBits / duration / 1000) - audioBitrateKbps);
  
  if (duration > 60) {
    const durationMultiplier = Math.min(1.2, 1 + (duration - 60) / 600);
    videoBitrateKbps = Math.floor(videoBitrateKbps * durationMultiplier);
  }

  let firstResult = await encodeVideo(videoBitrateKbps);
  ...
  try {
    ...
    const tolerance = targetBytes * 0.10;
    let attempts = 0;
    const maxAttempts = 2;
    
    while (size > targetBytes + tolerance && attempts < maxAttempts) {
      attempts++;
      videoBitrateKbps = Math.max(200, Math.floor(videoBitrateKbps * 0.80));
      console.log(`[Export] Retry ${attempts}: Target ${targetMB}MB, got ${(size / 1024 / 1024).toFixed(2)}MB, reducing bitrate to ${videoBitrateKbps}k`);
      ...
      const retryResult = await encodeVideo(videoBitrateKbps);
    ...
    if (size > targetBytes + tolerance && attempts >= maxAttempts) {
      console.warn(`[Export] Final size ${(size / 1024 / 1024).toFixed(2)}MB exceeds target ${targetMB}MB after ${maxAttempts} attempts`);
    }
  } catch (err) {
